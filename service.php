<?php include_once('./include/header.php') ?>
<section class="banner">
	<img src="assets/images/indore-banner.jpg" class="img-fluid" width="100%" height="400px">
</section>

<section class="bg-body-secondary services-section">
    <div class="container">
        <div class="row gy-4 align-items-center">
            <div class="col-md-6 col-lg-5 offset-lg-1">
                <img src="assets/images/top-rt-img.webp" alt="" class="img-fluid">
            </div>
            <div class="col-md-6">
                <h2 class="text-primary">About Lucknow Escorts Service</h2>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
            </div>
        </div>
    </div>
</section>
<section class="video-section services-section">
	<div class="container">
		<div class="row align-items-center">
			<div class="col-md-6">
                <h2 class="text-primary">About Lucknow Escorts Service</h2>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
            </div>
			<div class="col-md-6">
				<video autoplay="autoplay" controls="" muted="" loop="" playsinline="" width="100%">
                  <source src="https://www.alisachopra.in/images/alisachopravideo.mp4" type="video/mp4">
               </video>
			</div>
		</div>
	</div>
</section>
<section class="bg-body-secondary services-section">
    <div class="container">
        <div class="row gy-4 align-items-center">
            <div class="col-md-6 col-lg-5 offset-lg-1">
                <img src="assets/images/top-rt-img.webp" alt="" class="img-fluid">
            </div>
            <div class="col-md-6">
                <h2 class="text-primary">About Lucknow Escorts Service</h2>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
                <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Porro temporibus consectetur beatae aliquid iusto eius culpa laboriosam atque nisi deleniti obcaecati rem vero modi est recusandae officiis, dolor accusamus, placeat labore distinctio! Odio perferendis illo porro officia natus necessitatibus nostrum!</p>
            </div>
        </div>
    </div>
</section>
<section class="image-section">
	<img src="assets/images/banner-footer.webp" class="img-fluid" width="100%">
</section>
<?php include_once('./include/footer.php') ?>